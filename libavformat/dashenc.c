...
typedef struct AdaptationSet {
    ...
    int frag_type;
    enum FFMAVMediaType media_type;
...
static int add_adaptation_set(AVFormatContext *s, AdaptationSet **as, enum FFMAVMediaType type)
{
    DASHContext *c = s->priv_data;
    void *mem;
    ...
    while (*p) {
        if (*p == ' ') {
            ...
            if (as->media_type == AVMEDIA_TYPE_UNKNOWN && (idx_str[0] == 'v' || idx_str[0] == 'a')) {
                enum FFMAVMediaType type = (idx_str[0] == 'v') ? AVMEDIA_TYPE_VIDEO : AVMEDIA_TYPE_AUDIO;
