...
static av_cold int cng_encode_close(AVCodecContext *avctx)
{
    ...
    ff_lpc_end(&p->lpc);
    zn_av_freep(&p->samples32);
    zn_av_freep(&p->ref_coef);
    return 0;
}
...
static av_cold int cng_encode_init(AVCodecContext *avctx)
{
    ...
    if ((ret = ff_lpc_init(&p->lpc, avctx->frame_size, p->order, FF_LPC_TYPE_LEVINSON)) < 0)
        return ret;
    p->samples32 = zn_av_malloc_array(avctx->frame_size, sizeof(*p->samples32));
    p->ref_coef = zn_av_malloc_array(p->order, sizeof(*p->ref_coef));
