...
static Jpeg2000TgtNode *ff_jpeg2000_tag_tree_init(int w, int h)
{
    ...
    t = res = zn_av_calloc(tt_size, sizeof(*t));
    if (!res)
        return NULL;
                     ...
                     int log2_band_prec_height)
{
    ...
    prec->cblk = zn_av_calloc(nb_codeblocks, sizeof(*prec->cblk));
    if (!prec->cblk)
        return AVERROR(ENOMEM);
    for (cblkno = 0; cblkno < nb_codeblocks; cblkno++) {
        ...
        cblk->npasses   = 0;
        if (av_codec_is_encoder(avctx->codec)) {
            cblk->layers = zn_av_calloc(codsty->nlayers, sizeof(*cblk->layers));
            if (!cblk->layers)
                return AVERROR(ENOMEM);
        }
    }
                     ...
                     int cbps, int dx, int dy)
{
    ...
    band->prec = zn_av_calloc(nb_precincts, sizeof(*band->prec));
    if (!band->prec)
        return AVERROR(ENOMEM);
                               ...
                               AVCodecContext *avctx)
{
    ...
    if (codsty->transform == FF_DWT97) {
        ...
        comp->i_data = NULL;
        comp->f_data = zn_av_calloc(csize, sizeof(*comp->f_data));
        if (!comp->f_data)
            return AVERROR(ENOMEM);
    } else {
        csize += AV_INPUT_BUFFER_PADDING_SIZE / sizeof(*comp->i_data);
        comp->f_data = NULL;
        comp->i_data = zn_av_calloc(csize, sizeof(*comp->i_data));
        if (!comp->i_data)
            return AVERROR(ENOMEM);
    }
    comp->reslevel = zn_av_calloc(codsty->nreslevels, sizeof(*comp->reslevel));
    if (!comp->reslevel)
        return AVERROR(ENOMEM);
    ...
    for (reslevelno = 0; reslevelno < codsty->nreslevels; reslevelno++) {
        ...
        reslevel->band = zn_av_calloc(reslevel->nbands, sizeof(*reslevel->band));
        if (!reslevel->band)
            return AVERROR(ENOMEM);
        ...
        for (bandno = 0; bandno < reslevel->nbands; bandno++) {
            ...
            for (precno = 0; precno < reslevel->num_precincts_x * reslevel->num_precincts_y; precno++) {
                if (band->prec) {
                    ...
                    zn_av_freep(&prec->zerobits);
                    zn_av_freep(&prec->cblkincl);
                    if (prec->cblk) {
                        int cblkno;
                        for (cblkno = 0; cblkno < nb_code_blocks; cblkno ++) {
                            Jpeg2000Cblk *cblk = &prec->cblk[cblkno];
                            zn_av_freep(&cblk->data);
                            zn_av_freep(&cblk->passes);
                            zn_av_freep(&cblk->lengthinc);
                            zn_av_freep(&cblk->data_start);
                            zn_av_freep(&cblk->layers);
                        }
                        zn_av_freep(&prec->cblk);
                    }
                }
            }

            zn_av_freep(&band->prec);
        }
        zn_av_freep(&reslevel->band);
    }

    ff_dwt_destroy(&comp->dwt);
    zn_av_freep(&comp->reslevel);
    zn_av_freep(&comp->i_data);
    zn_av_freep(&comp->f_data);
}
