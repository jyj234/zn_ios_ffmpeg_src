...
void checkasm_check_nlmeans(void)
{
    ...
    if (check_func(dsp.compute_safe_ssd_integral_image, "ssd_integral_image")) {
        ...
        uint32_t *ii_orig_ref = zn_av_calloc(ii_h + 1, ii_lz_32 * sizeof(*ii_orig_ref));
        uint32_t *ii_ref = ii_orig_ref + ii_lz_32 + 1;
        uint32_t *ii_orig_new = zn_av_calloc(ii_h + 1, ii_lz_32 * sizeof(*ii_orig_new));
        uint32_t *ii_new = ii_orig_new + ii_lz_32 + 1;
        const int src_lz = FFALIGN(w, 16);
        uint8_t *src = zn_av_calloc(h, src_lz);
        ...
        zn_av_freep(&ii_orig_ref);
        zn_av_freep(&ii_orig_new);
        zn_av_freep(&src);
    }
