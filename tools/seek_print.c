...
int main(int argc, char **argv)
{
    ...
    if ((ret = zn_avformat_open_input(&avf, filename, NULL, NULL)) < 0) {
        fprintf(stderr, "%s: %s\n", filename, zn_av_err2str(ret));
        return 1;
    }
    if ((ret = zn_avformat_find_stream_info(avf, NULL)) < 0) {
    ...
    for (; argc; argc--, argv++) {
        if (!strcmp(*argv, "read")) {
            ret = zn_av_read_frame(avf, &packet);
            if (ret < 0) {
                printf("read: %d (%s)\n", ret, zn_av_err2str(ret));
            } else {
