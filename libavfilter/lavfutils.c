                  ...
                  const char *filename, void *log_ctx)
{
    ...
    if ((ret = zn_avformat_open_input(&format_ctx, filename, iformat, NULL)) < 0) {
        av_log(log_ctx, AV_LOG_ERROR,
               "Failed to open input file '%s'\n", filename);
        return ret;
    }

    if ((ret = zn_avformat_find_stream_info(format_ctx, NULL)) < 0) {
        av_log(log_ctx, AV_LOG_ERROR, "Find stream info failed\n");
        goto end;
    }

    par = format_ctx->streams[0]->codecpar;
    codec = zn_avcodec_find_decoder(par->codec_id);
    if (!codec) {
    ...
    codec_ctx = zn_avcodec_alloc_context3(codec);
    if (!codec_ctx) {
    ...
    ret = zn_avcodec_parameters_to_context(codec_ctx, par);
    if (ret < 0) {
    ...
    if ((ret = zn_avcodec_open2(codec_ctx, codec, &opt)) < 0) {
    ...
    ret = zn_av_read_frame(format_ctx, &pkt);
    if (ret < 0) {
        av_log(log_ctx, AV_LOG_ERROR, "Failed to read frame from file\n");
        goto end;
    }

    ret = zn_avcodec_send_packet(codec_ctx, &pkt);
    ...
    ret = zn_avcodec_receive_frame(codec_ctx, frame);
    if (ret < 0) {
    ...
    *pix_fmt = frame->format;
    ...
    zn_avformat_close_input(&format_ctx);
