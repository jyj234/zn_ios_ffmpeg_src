...
static int init_input_filter(FilterGraph *fg, InputFilter *ifilter)
{
    ...
    enum FFMAVMediaType type = ifp->type;
    int i, ret;
    ...
    if (ifp->linklabel) {
        ...
        for (i = 0; i < s->nb_streams; i++) {
            enum FFMAVMediaType stream_type = s->streams[i]->codecpar->codec_type;
            if (stream_type != type &&
                       ...
                       const char *filter_name)
{
    ...
    AVFilterContext *ctx;
    const AVFilter *trim;
    enum FFMAVMediaType type = avfilter_pad_get_type((*last_filter)->output_pads, *pad_idx);
...
static int fg_output_frame(OutputFilterPriv *ofp, AVFrame *frame)
{
    ...
    enum FFMAVMediaType type = ofp->ofilter.type;
